...
- **One `AsyncClient` per transaction** (created in `start_transaction_()`, destroyed in `finish_transaction_()`) — never reuse a client across connections. RPAsyncTCP (RP2040) `close()` only *defers* the close: a reused client still holds the old pcb, refuses the next `connect()`, and closing again touches freed memory (hard fault, verified on a Pico W). The destructor is the one immediate, callback-clearing close all the AsyncTCP libraries implement correctly.
